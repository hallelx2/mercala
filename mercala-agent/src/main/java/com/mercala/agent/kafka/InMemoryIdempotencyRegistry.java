...
/**
 * Bounded in-memory thread-safe registry to deduplicate consumed events.
 * Used for database-less microservices where persistent storage is unavailable.
 */
@Component
    ...
    private final Set<UUID> processedEvents = Collections.synchronizedSet(
        Collections.newSetFromMap(new LinkedHashMap<UUID, Boolean>() {
            ...
            protected boolean removeEldestEntry(Map.Entry<UUID, Boolean> eldest) {
                return size() > 10000; // Cap registry to prevent unbounded memory growth
            }
        })
    );
