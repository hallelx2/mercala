...
public class PaystackPaymentProvider implements PaymentProvider {
    ...
    @Override
    @CircuitBreaker(name = "paystack-payment", fallbackMethod = "fallbackInitialize")
    @Retry(name = "paystack-payment")
    ...
    @Override
    @CircuitBreaker(name = "paystack-payment", fallbackMethod = "fallbackVerify")
    @Retry(name = "paystack-payment")
    ...
    @Override
    @CircuitBreaker(name = "paystack-payment", fallbackMethod = "fallbackRefund")
    @Retry(name = "paystack-payment")
    ...
    /**
     * Fallbacks for Paystack payment provider operations.
     */
    public PaymentResponse fallbackInitialize(PaymentRequest request, Throwable t) {
        log.error("Paystack payment initialization failed or circuit is open. Fallback triggered. Error: {}", t.getMessage());
        return PaymentResponse.failed("Paystack service temporarily unavailable: " + t.getMessage());
    }

    public PaymentResponse fallbackVerify(String transactionReference, Throwable t) {
        log.error("Paystack payment verification failed or circuit is open. Fallback triggered. Error: {}", t.getMessage());
        return PaymentResponse.failed("Paystack verification service temporarily unavailable: " + t.getMessage());
    }

    public RefundResponse fallbackRefund(RefundRequest request, Throwable t) {
        log.error("Paystack refund failed or circuit is open. Fallback triggered. Error: {}", t.getMessage());
        return RefundResponse.failed("Paystack refund service temporarily unavailable: " + t.getMessage());
    }
}
